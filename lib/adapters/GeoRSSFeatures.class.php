 ...
 */
class GeoRSSFeayures extends FeatureAdapter
{
  ...
  protected function featuresFromXML() {
    $features = array();
    $features = array_merge($features, $this->parseFeatures());
  ...
  protected function parseFeatures() {
    $features = array();
    $feature_elements = $this->xmlobj->getElementsByTagName('item');
    foreach ($feature_elements as $feature) {
        $geometry = 
        $features[] = new Feature($geometry, $properties, $id);
    }
